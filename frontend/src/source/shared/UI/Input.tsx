...
export const Input = forwardRef<HTMLInputElement, Props>((props, ref) => {
  ...
  return (
    <label className='flex w-full group'>
      {Icon && (
        <div className='mr-3'>
      ...
      <input
        {...rest}
        className={cn("border-none autofill:bg-none bg-transparent outline-none placeholder:text-[#666463]", className)}
        type={initialType}
        ref={ref}
      />
      {error && <div className='text-red-600 text-sm bg-transparent'>{error?.message}</div>}
    </label>
  );
});
